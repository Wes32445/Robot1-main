...
ez::Drive chassis(
  ...
  // Motor on port 19, gearset green
  // Note: PROS Motor constructor uses the sign of the port to indicate reversal.
  pros::Motor motor(19);
...
void initialize() {
  ...
  // Optional: set integration time (how long the sensor averages light)
  optical.set_integration_time(50); // 50 ms is a reasonable starting point
...
void opcontrol() {
  ...
  while (true) {
    // ---------------- DRIVER CONTROL ----------------
    chassis.opcontrol_tank();   // Tank drive
    //intake_opcontrol();       // If you use this instead of intake_apply()
    intake_apply();
    ...
    if (hue < 0.0) {
      ...
      if (is_red) {
        motor.move(80);      // forward on red
      } else if (is_blue) {
        motor.move(-80);     // reverse on blue
      } else {
        motor.move(0);       // stop on other colors
      }
    }

    // ---------------- LOOP DELAY ----------------
    pros::delay(ez::util::DELAY_TIME);  // keeps EZ-Template happy
  }
}
